...
-- ROLE use session_user.  An unprivileged caller cannot SET ROLE to a role it
...
DECLARE
    ...
    v_chunk_size bigint := pgs3._setting_bytes('pgs3.chunk_size', 1048576);
...
BEGIN
    ...
    BEGIN
        ...
        INSERT INTO pgs3.object (
            bucket_id, key, version_id, is_latest, delete_marker, size, etag,
            sha256, content_type, meta, inline, blob_id, created_at, created_by
        )
    ...
    -- This is the set-wise equivalent of object_blob_refs_insert.  Aggregate
    -- from the rows actually copied into the destination, so concurrent source
    -- writes cannot skew canonical ownership counts.
    UPDATE pgs3.blob AS b
       SET refcount = b.refcount + refs.n
      FROM (
          SELECT o.blob_id, count(*)::bigint AS n
            FROM pgs3.object AS o
           WHERE o.bucket_id = v_destination_bucket_id
             AND o.blob_id IS NOT NULL
           GROUP BY o.blob_id
      ) AS refs
...
BEGIN
    ...
    IF NOT FOUND OR v_target.rolsuper OR v_target.rolbypassrls
    ...
    IF NOT FOUND OR v_server_role.rolsuper OR v_server_role.rolbypassrls
...
BEGIN
    ...
    THEN
        RAISE EXCEPTION USING
            ERRCODE = '42501',
            MESSAGE = 'existing pgs3_server role is not a restricted service role';
    END IF;
END
$pgs3_server_role$;
...
BEGIN
    EXECUTE format(
        'GRANT CONNECT ON DATABASE %I TO pgs3_server',
        current_database()
    );
END
$pgs3_server_grants$;

GRANT USAGE ON SCHEMA pgs3 TO pgs3_server;
GRANT SELECT ON TABLE pgs3.credential TO pgs3_server;
GRANT EXECUTE ON FUNCTION pgs3._worker_set_state(text, integer, integer, text, text, integer, text)
    TO pgs3_server;
GRANT EXECUTE ON FUNCTION pgs3._worker_add_metric(text, integer, text, bigint, bigint, bigint, bigint, bigint, bigint, bigint, bigint, bigint, bigint, bigint, bigint, bigint)
    TO pgs3_server;
...
BEGIN
    ...
    IF NEW.role_name <> v_server THEN
        EXECUTE format(
            'GRANT %I TO %I WITH SET TRUE, INHERIT FALSE',
            NEW.role_name,
            v_server
...
/* <begin connected objects> */
-- src/hash.rs:615
CREATE SCHEMA IF NOT EXISTS pgs3; /* pgs3::hash::pgs3 */
/* </end connected objects> */

/* <begin connected objects> */
-- src/hash.rs:667
-- pgs3::hash::pgs3::hash_blob_sequence
CREATE  FUNCTION pgs3."hash_blob_sequence"(
	"blob_ids" bytea[] /* Array < '_, & [u8] > */
) RETURNS TABLE (
	"sha256" bytea,  /* Vec < u8 > */
	"total_size" bigint  /* i64 */
)
STRICT STABLE SECURITY DEFINER
SET search_path TO pg_catalog, pgs3
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'hash_blob_sequence_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/hash.rs:620
-- pgs3::hash::pgs3::hash_upload_part
CREATE  FUNCTION pgs3."hash_upload_part"(
	"upload_id" uuid, /* Uuid */
	"part_number" INT /* i32 */
) RETURNS TABLE (
	"sha256" bytea,  /* Vec < u8 > */
...
SET search_path TO pg_catalog, pgs3
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'hash_upload_part_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/api.rs:1
CREATE SCHEMA IF NOT EXISTS pgs3; /* pgs3::api::pgs3 */
/* </end connected objects> */

/* <begin connected objects> */
-- src/api.rs:13
-- pgs3::api::pgs3::start
CREATE  FUNCTION pgs3."start"() RETURNS bool /* bool */
...
/* <begin connected objects> */
-- src/api.rs:7
-- pgs3::api::pgs3::sha256
CREATE  FUNCTION pgs3."sha256"(
	"value" bytea /* & [u8] */
) RETURNS bytea /* Vec < u8 > */
IMMUTABLE STRICT PARALLEL SAFE
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'sha256_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/api.rs:20
-- pgs3::api::pgs3::stop
CREATE  FUNCTION pgs3."stop"() RETURNS bool /* bool */
STRICT SECURITY DEFINER
SET search_path TO pg_catalog, pgs3
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'stop_wrapper';
/* </end connected objects> */
